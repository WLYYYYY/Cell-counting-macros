...
if (Selection1=="2") {
	...
	Relations = newArray("And (overlapping A & B)", "Or (either A or B)", "Subtract (B minus A)", "Difference (B minus A or A minus B)", "Copy (express channel A ROIs in channel B)", "XOR (mutually exclusive)");
	...
	if (sRelat=="Subtract (B minus A)")
		imgcalop = "subtract";
	if (sRelat=="Difference (B minus A or A minus B)")
		imgcalop = "difference";
	if (sRelat=="Copy (express channel A ROIs in channel B)")
		imgcalop = "copy";
